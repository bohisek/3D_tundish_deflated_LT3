...
// advection of ux
template <class T>
__global__ void advectUx(T *dux, T *duxhalf, T *duyhalf, T*duzhalf, T *duxo, T *duyo, T*duzo)
{
	int i = threadIdx.x + blockIdx.x * blockDim.x;
	int nx = d_dims.Nx+2;
	int ny = d_dims.Ny+2;
	...
	if ((px>0) && (py>0) && (pz>0) && (px<nx-2) && (py<ny-1) && (pz<nz-1)) { // skip boundary
		...
		y = py - 0.5 * dt * 0.25 * (duyhalf[i] + duyhalf[i+1] + duyhalf[i-nx]    + duyhalf[i-nx+1]   ) / dx;
		z = pz - 0.5 * dt * 0.25 * (duzhalf[i] + duzhalf[i+1] + duzhalf[i-nx*ny] + duzhalf[i+1-nx*ny]) / dx;
		...
		// setup trilinear interpolation "corner points" for DUX
		...
		ux = dx0 * (dy0 * (dz0 * duxhalf[px0+nx*py0+nx*ny*pz0] + dz1 * duxhalf[px0+nx*py0+nx*ny*pz1])
				  + dy1 * (dz0 * duxhalf[px0+nx*py1+nx*ny*pz0] + dz1 * duxhalf[px0+nx*py1+nx*ny*pz1]))
		   + dx1 * (dy0 * (dz0 * duxhalf[px1+nx*py0+nx*ny*pz0] + dz1 * duxhalf[px1+nx*py0+nx*ny*pz1])
				  + dy1 * (dz0 * duxhalf[px1+nx*py1+nx*ny*pz0] + dz1 * duxhalf[px1+nx*py1+nx*ny*pz1]));

		// setup trilinear interpolation "corner points" for DUY
		px0 = (int) (x+0.5); px1 = px0 + 1; dx1 = x+0.5-px0; dx0 = 1 - dx1;
		py0 = (int) (y-0.5); py1 = py0 + 1; dy1 = y-0.5-py0; dy0 = 1 - dy1;
		pz0 = (int)  z     ; pz1 = pz0 + 1; dz1 = z    -pz0; dz0 = 1 - dz1;

		uy = dx0 * (dy0 * (dz0 * duyhalf[px0+nx*py0+nx*ny*pz0] + dz1 * duyhalf[px0+nx*py0+nx*ny*pz1])
				  + dy1 * (dz0 * duyhalf[px0+nx*py1+nx*ny*pz0] + dz1 * duyhalf[px0+nx*py1+nx*ny*pz1]))
		   + dx1 * (dy0 * (dz0 * duyhalf[px1+nx*py0+nx*ny*pz0] + dz1 * duyhalf[px1+nx*py0+nx*ny*pz1])
				  + dy1 * (dz0 * duyhalf[px1+nx*py1+nx*ny*pz0] + dz1 * duyhalf[px1+nx*py1+nx*ny*pz1]));

		// setup trilinear interpolation "corner points" for DUZ
		px0 = (int) (x+0.5); px1 = px0 + 1; dx1 = x+0.5-px0; dx0 = 1 - dx1;
		py0 = (int)  y     ; py1 = py0 + 1; dy1 = y    -py0; dy0 = 1 - dy1;
		pz0 = (int) (z-0.5); pz1 = pz0 + 1; dz1 = z-0.5-pz0; dz0 = 1 - dz1;

		uz = dx0 * (dy0 * (dz0 * duzhalf[px0+nx*py0+nx*ny*pz0] + dz1 * duzhalf[px0+nx*py0+nx*ny*pz1])
				  + dy1 * (dz0 * duzhalf[px0+nx*py1+nx*ny*pz0] + dz1 * duzhalf[px0+nx*py1+nx*ny*pz1]))
		   + dx1 * (dy0 * (dz0 * duzhalf[px1+nx*py0+nx*ny*pz0] + dz1 * duzhalf[px1+nx*py0+nx*ny*pz1])
				  + dy1 * (dz0 * duzhalf[px1+nx*py1+nx*ny*pz0] + dz1 * duzhalf[px1+nx*py1+nx*ny*pz1]));
		...
		// perform a trilinear interpolation
		dux[i] = dx0 * (dy0 * (dz0 * duxo[px0+nx*py0+nx*ny*pz0] + dz1 * duxo[px0+nx*py0+nx*ny*pz1])
					  + dy1 * (dz0 * duxo[px0+nx*py1+nx*ny*pz0] + dz1 * duxo[px0+nx*py1+nx*ny*pz1]))
			   + dx1 * (dy0 * (dz0 * duxo[px1+nx*py0+nx*ny*pz0] + dz1 * duxo[px1+nx*py0+nx*ny*pz1])
					  + dy1 * (dz0 * duxo[px1+nx*py1+nx*ny*pz0] + dz1 * duxo[px1+nx*py1+nx*ny*pz1]));
	}
}

// advection of uy
template <class T>
__global__ void advectUy(T *duy, T *duxhalf, T *duyhalf, T*duzhalf, T *duxo, T *duyo, T*duzo)
{
	int i = threadIdx.x + blockIdx.x * blockDim.x;
	int nx = d_dims.Nx+2;
	int ny = d_dims.Ny+2;
	...
	if ((px>0) && (py>0) && (pz>0) && (px<nx-1) && (py<ny-2) && (pz<nz-1)) { // skip boundary
		...
		x = px - 0.5 * dt * 0.25 * (duxhalf[i] + duxhalf[i-1]     + duxhalf[i+nx] + duxhalf[i+nx-1]) / dx;
		y = py - 0.5 * dt * duyhalf[i] / dx;
		z = pz - 0.5 * dt * 0.25 * (duzhalf[i] + duzhalf[i-nx*ny] + duzhalf[i+nx] + duzhalf[i+nx-nx*ny]) / dx;
		...
		// perform a trilinear interpolation
		duy[i] = dx0 * (dy0 * (dz0 * duyo[px0+nx*py0+nx*ny*pz0] + dz1 * duyo[px0+nx*py0+nx*ny*pz1])
					  + dy1 * (dz0 * duyo[px0+nx*py1+nx*ny*pz0] + dz1 * duyo[px0+nx*py1+nx*ny*pz1]))
			   + dx1 * (dy0 * (dz0 * duyo[px1+nx*py0+nx*ny*pz0] + dz1 * duyo[px1+nx*py0+nx*ny*pz1])
					  + dy1 * (dz0 * duyo[px1+nx*py1+nx*ny*pz0] + dz1 * duyo[px1+nx*py1+nx*ny*pz1]));
	}
}
...
// advection of uz
template <class T>
__global__ void advectUz(T *duz, T *duxhalf, T *duyhalf, T*duzhalf, T *duxo, T *duyo, T*duzo)
{
	int i = threadIdx.x + blockIdx.x * blockDim.x;
	int nx = d_dims.Nx+2;
	int ny = d_dims.Ny+2;
	...
	if ((px>0) && (py>0) && (pz>0) && (px<nx-1) && (py<ny-1) && (pz<nz-2)) { // skip boundary
		...
		// move "backwards" in time
		x = px - 0.5 * dt * 0.25 * (duxhalf[i] + duxhalf[i-1]  + duxhalf[i+nx*ny] + duxhalf[i+nx*ny-1]) / dx;
		y = py - 0.5 * dt * 0.25 * (duyhalf[i] + duyhalf[i-nx] + duyhalf[i+nx*ny] + duyhalf[i+nx*ny-nx]) / dx;
		...
		// perform a trilinear interpolation
		duz[i] = dx0 * (dy0 * (dz0 * duzo[px0+nx*py0+nx*ny*pz0] + dz1 * duzo[px0+nx*py0+nx*ny*pz1])
					  + dy1 * (dz0 * duzo[px0+nx*py1+nx*ny*pz0] + dz1 * duzo[px0+nx*py1+nx*ny*pz1]))
			   + dx1 * (dy0 * (dz0 * duzo[px1+nx*py0+nx*ny*pz0] + dz1 * duzo[px1+nx*py0+nx*ny*pz1])
					  + dy1 * (dz0 * duzo[px1+nx*py1+nx*ny*pz0] + dz1 * duzo[px1+nx*py1+nx*ny*pz1]));
