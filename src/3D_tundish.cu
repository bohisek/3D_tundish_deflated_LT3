...
int main()
{
	...
	*/
	...
	for (int miter=0; miter<params.steps; miter++) {
		...
		advectUx<<<BLOCKS_NEW,THREADS_PER_BLOCK_NEW>>>(dux, duxhalf, duyhalf, duzhalf, duxo, duyo, duzo);
		advectUy<<<BLOCKS_NEW,THREADS_PER_BLOCK_NEW>>>(duy, duxhalf, duyhalf, duzhalf, duxo, duyo, duzo);
		advectUz<<<BLOCKS_NEW,THREADS_PER_BLOCK_NEW>>>(duz, duxhalf, duyhalf, duzhalf, duxo, duyo, duzo);
		...
		while (iterSIMPLE < params.maxIterSIMPLE) {  // (rhNewSIMPLE > params.maxResSIMPLE) {   
		...
		cudaMemcpy(uz, duz, sizeof(T)*(dims.Nx+2)*(dims.Ny+2)*(dims.Nz+2), cudaMemcpyDeviceToHost);
		cudaMemcpy(p,  dpo, sizeof(T)*dims.Nx    * dims.Ny   *(dims.Nz+2), cudaMemcpyDeviceToHost);
		saveDataInTime(ux, uy, uz, p, m, (T)miter, "3Dtundish_LT3_CFL1");
